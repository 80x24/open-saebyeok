...
> 봇 설치 경로: `cat ~/.nuanua/.nuanua-path` 로 확인 (보통 `~/nuanua`). 아래 `<설치경로>` 는 이 값입니다.
...
- 회사·팀에서 슬랙을 쓰면 **슬랙**.
- **둘 다 동시에**도 됩니다 — 받은 채널로 답하고 기억은 공유돼요. 그땐 `CHANNEL=telegram,slack` (콤마), 양쪽 토큰을 모두 받으세요.
...
받은 값을 `<설치경로>/bot/.env` 에 적습니다 (데이터 폴더 `~/.nuanua/` 가 아니라 봇 폴더예요).
